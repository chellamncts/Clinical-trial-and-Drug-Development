...
function userTable(d,withActions=false){return d.length?`<table><tr><th>#</th><th>Username</th><th>Name</th><th>Email</th><th>Role</th>${withActions?'<th>Action</th>':''}</tr>`+d.map(u=>`<tr><td><span class="ig-v mut">${u.id}</span></td><td><b>${u.username}</b></td><td>${u.fullName||"—"}</td><td>${u.email?`<a href="mailto:${u.email}">${u.email}</a>`:"—"}</td><td><span class="badge b-approved">${pretty(u.role)}</span></td>${withActions?`<td><button class="btn logout" onclick="deleteUser(${u.id})"><i class="bi bi-person-dash"></i> Deactivate</button></td>`:''}</tr>`).join("")+"</table>":'<div class="empty">No users yet</div>';}
