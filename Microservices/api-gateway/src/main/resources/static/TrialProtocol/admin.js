...
function protoLabel(p){return p.trialTitle;}
function siteLabel(s){return s.siteName;}
...
async function createUser(){
  ...
  try{await api("/users","POST",{username,fullName:u_fullname.value,password,role});umsg.innerHTML='<div class="msg ok">User created</div>';u_username.value=u_fullname.value=u_password.value="";loadUsers();setTimeout(()=>toggleForm('userForm'),700);}catch(e){umsg.innerHTML='<div class="msg err">'+e.message+'</div>';}}
...
// Friendlier lifecycle error (server returns 409 on invalid transition)
...
// Enable only the valid next lifecycle action for the selected protocol
...
// Enable Activate Site only when the site is REGISTERED and its protocol is APPROVED/ACTIVE
